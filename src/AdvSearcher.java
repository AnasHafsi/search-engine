...
public class AdvSearcher {
	...
	public static List<Engine> enginesFeed() {
		...
		Engine analyzerM = new Engine("Stemmer with stopwords", new AnalyzerM(), new BM25Similarity(),
				INPUT_PATH + "analyzerM/", OUTPUT_PATH + "AnalyzerM.txt");
		Engine analyzerA = new Engine("(K) Stemmer without stopwords", new AnalyzerA(), new BM25Similarity(),
				INPUT_PATH + "analyzerA/", OUTPUT_PATH + "AnalyzerA.txt");
		Engine simpleStopWordsWithBM25 = new Engine("Simple Words with BM25Similarity", new EnglishAnalyzer(),
				new BM25Similarity(), INPUT_PATH + "simplestopwords/", OUTPUT_PATH + "simple_stop_words.txt");
		Engine standard = new Engine("Standard with BM25Similarity", new StandardAnalyzer(), new BM25Similarity(),
				INPUT_PATH + "standard/", OUTPUT_PATH + "standard.txt");
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(
				new Date().toLocaleString() + ": Launching Advanced Searcher for " + engines.size() + " engines");
		System.out.println();
	 ...
	 */
	public static String doBatchSearch(BufferedReader in, IndexSearcher searcher, String qid, Query query,
		...
		for (int i = start; i < end; i++) {
			...
			// There are duplicate document numbers in the FR collection, so only output a
			// given
			// docno once.
